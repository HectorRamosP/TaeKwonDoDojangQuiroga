/** @module components/modals/ModalSocio */
import {
    TextField,
    FormControl,
    InputLabel,
 ...
 * @param {Function} props.recargar - Callback para recargar la lista de alumnos.
 * @param {"crear"|"editar"} [props.modo="crear"] - Modo del modal.
 * @param {object} [props.socio] - Datos del alumno a editar (solo en modo editar).
 */
export default function ModalSocio({ abierto, cerrar, recargar, modo = "crear", socio }) {
    ...
    useEffect(() => {
        if (esEditar && socio && cintas.length > 0 && clases.length > 0 && conceptos.length > 0) {
    ...
    }, [esEditar, socio, cintas, clases, conceptos, reset]);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            manejarErrorApi(error, esEditar ? "actualizar el alumno" : "guardar el alumno");
        } finally {
            setGuardando(false);
        }
    };
    ...
    return (
        <ModernModal
            open={abierto}
            onClose={handleClose}
            title={esEditar ? "Editar Alumno" : "Nuevo Alumno"}
            icon={esEditar ? <Edit /> : <PersonAdd />}
            maxWidth="md"
            formId="form-socio"
            loading={guardando}
        >
            <form id="form-socio" onSubmit={handleSubmit(onSubmit)}>
                ...
                <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
                    <TextField
                        ...
                        disabled={guardando}
                        autoFocus={!esEditar}
                        InputLabelProps={esEditar ? { shrink: true } : undefined}
                    />
                    <TextField
                ...
                <Typography variant="subtitle2" sx={{ mt: 3, mb: 2, color: "#666", fontWeight: "bold" }}>
                    Información de Entrenamiento {!esEditar && "(Opcional)"}
                </Typography>
                <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
                    <FormControl fullWidth disabled={guardando}>
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <Select {...field} label="Cinta Actual" value={field.value || ""}>
                                    <MenuItem value=""><em>Sin cinta asignada</em></MenuItem>
                                    {cintas.map((cinta) => (
                                        <MenuItem key={cinta.id} value={cinta.id}>{cinta.nombre}</MenuItem>
                                    ))}
                                </Select>
                            )}
                        />
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <Select {...field} label="Mensualidad Contratada" value={field.value || ""}>
                                    <MenuItem value=""><em>Sin mensualidad</em></MenuItem>
